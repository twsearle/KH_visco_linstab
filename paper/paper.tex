...
%	Last modified: Fri 12 Feb 17:42:28 2016
%	using jfm template
...
\begin{figure}[h]
    ...
    \caption{Diagram of a shear layer} 
...
pseudospectral domain. To validate the code, we examine the high Reynolds
number regime. We see the height of the instability decrease with increasing
...
The instability remains present under variation of $\Delta$ at low $\Delta$. At
$\Delta =0.1$ the instability is still present for $\Rey < 0.01$ far lower than
the usual threshold of $\Rey \sim 1000$ for a Newtonian Kelvin-Helmholtz
instability (figure \ref{fig:KH_walls_vary_Re}).
...
observe (\citet{Azaiez1994} figure 8). Nonetheless at least one of their
observations does apply. The fact that the velocity disturbance is essentially
zero in the center of the channel, implies that the shear layer will be less
likely to roll up. In the Newtonian instability, net velocities within the
shear layer will distort its shape, however the purely elastic instability
appears to have less of an effect on the shear layer.
